...
void attPosCaixa(double* boxInitialPosition, double* boxCurrentPosition);
// node Caixa
...
int main(int argc, char **argv) {
  ...
  WbNodeRef node_robo = wb_supervisor_node_get_from_def("node");
  WbFieldRef field_robo = wb_supervisor_node_get_field(node_robo, "translation");
...
void defCaixaPos(WbFieldRef* caixa_def, double** caixa_posicao){
  for(int i = 0; i < 9; i++){
    double* posicao_box = wb_supervisor_field_get_sf_vec3f(caixa_def[i]);
    for(int j = 0; j < 3; j++)
...
void verificarColisao(WbDeviceTag* sensores_proximidade, WbFieldRef* caixa_def, double** caixa_posicao, WbDeviceTag* leds){
  ...
  if (objeto_detectado){
     for(int i = 0; i < nr_caixas; i++){
        double* posicaoBox = wb_supervisor_field_get_sf_vec3f(caixa_def[i]);
        for(int j = 0; j < 3; j++){
          if(posicaoBox[j] != caixa_posicao[i][j]){        
            objeto_movel = true;
            attPosCaixa(caixa_posicao[i],posicaoBox);
            break;
          }  
        } 
     }
...
void verificarObstaculos(WbDeviceTag* sensores_proximidade, sensor_obstaculos* sensor_obstaculos){
  ...
  bool direita =
      valor_sensores_proximidade[0] > 80.0 ||
      valor_sensores_proximidade[1] > 80.0 ||
      valor_sensores_proximidade[2] > 80.0;
      
  bool esquerda =
      valor_sensores_proximidade[5] > 80.0 ||
      valor_sensores_proximidade[6] > 80.0 ||
      valor_sensores_proximidade[7] > 80.0;
